...
interface DesignStoreContextType {
  ...
  updatePropellers: (
    propellers: Partial<DroneDesignConfig["propellers"]>,
  ) => void;
  updateBattery: (battery: Partial<DroneDesignConfig["battery"]>) => void;
  updateFlightController: (
    fc: Partial<DroneDesignConfig["flightController"]>,
  ) => void;
...
const DesignStoreContext = createContext<DesignStoreContextType | undefined>(
  undefined,
);
...
export function DesignStoreProvider({ children }: { children: ReactNode }) {
  ...
  const updateFrame = useCallback(
    (frame: Partial<DroneDesignConfig["frame"]>) => {
      setDesign((prev) => ({
        ...prev,
  ...
  const updateMotors = useCallback(
    (motors: Partial<DroneDesignConfig["motors"]>) => {
      setDesign((prev) => ({
        ...prev,
  ...
  const updatePropellers = useCallback(
    (propellers: Partial<DroneDesignConfig["propellers"]>) => {
      setDesign((prev) => ({
        ...prev,
  ...
  const updateBattery = useCallback(
    (battery: Partial<DroneDesignConfig["battery"]>) => {
      setDesign((prev) => ({
        ...prev,
  ...
  const updateFlightController = useCallback(
    (fc: Partial<DroneDesignConfig["flightController"]>) => {
      setDesign((prev) => ({
        ...prev,
  ...
  const updateCamera = useCallback(
    (camera: Partial<DroneDesignConfig["camera"]>) => {
      setDesign((prev) => ({
        ...prev,
